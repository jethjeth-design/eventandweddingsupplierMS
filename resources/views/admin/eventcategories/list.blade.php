...
<div class="page-content">
    ...
    <div class="bv-page-header">
        <div>
            <h1 class="bv-page-title">Event <em>Categories</em></h1>
            <p class="bv-page-sub">Manage Categories Event types</p>
        </div>
    ...
    <div class="bv-card">
        ...
        <table class="bv-table">
            ...
            <tbody>
                @foreach($eventcategories as $i => $eventcategories)
                <tr>
                    <td><span class="bv-row-num">{{ $i + 1 }}</span></td>
                    <td>
                        <div class="bv-eventcategories-name">
                            <span class="bv-eventcategories-dot"></span>
                            {{ $eventcategories->name }}
                        </div>
                    </td>
                    <td>
                        <span class="bv-eventcategories-desc" title="{{ $eventcategories->description }}">
                            {{ $eventcategories->description ?: '—' }}
                        </span>
                    </td>
                    <td>
                        <div class="bv-actions">
                            <a href="{{ route('admin.event.edit', $eventcategories->id) }}" class="bv-btn-edit">
                                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
                                    <path d="M11.5 2.5l2 2L5 13H3v-2L11.5 2.5z"/>
                                </svg>
                                Edit
                            </a>
                            <form method="POST" action="{{ route('admin.event.destroy', $eventcategories->id) }}" style="display:inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bv-btn-delete"
                                    onclick="return confirm('Delete {{ addslashes($eventcategories->name) }}?')">
                                    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
        ...
        <div class="bv-empty">
            ...
            <div class="bv-empty-title">No Events yet</div>
            <p class="bv-empty-sub">Click <strong>Add Event</strong> to create your first event Event.</p>
        </div>
        @endif
    </div>
...
{{-- Add Event Modal --}}
<div id="categoryModal" class="bv-modal-backdrop">
    <div class="bv-modal">
        <div class="bv-modal-header">
            <span class="bv-modal-title">Add <em>Event Category</em></span>
        ...
        <form method="POST" action="{{ route('admin.event.store') }}">
            ...
            <div class="bv-modal-body">
                ...
                <div class="bv-field">
                    <label class="bv-label" for="eventName">Event Type Name</label>
                    <input id="eventName" name="name" type="text"
                           class="bv-input" placeholder="e.g. Garden Wedding, Corporate Gala…"
                           required>
                </div>

                <div class="bv-field" style="margin-bottom:0">
                    <label class="bv-label" for="eventDesc">Description</label>
                    <textarea id="eventDesc" name="description"
                              class="bv-textarea"
                              placeholder="Briefly describe this event"></textarea>
                </div>
