...
<div class="page-content">
    ...
    <div class="bv-page-header">
        <div>
            <h1 class="bv-page-title">Supplier <em>Categories</em></h1>
            <p class="bv-page-sub">Manage categories and theme types for events</p>
        </div>
    ...
    <div class="bv-card">
        ...
        <table class="bv-table">
            ...
            <tbody>
                @foreach($categories as $i => $categories)
                <tr>
                    <td><span class="bv-row-num">{{ $i + 1 }}</span></td>
                    <td>
                        <div class="bv-categories-name">
                            <span class="bv-categories-dot"></span>
                            {{ $categories->name }}
                        </div>
                    </td>
                    <td>
                        <span class="bv-categories-desc" title="{{ $categories->description }}">
                            {{ $categories->description ?: '—' }}
                        </span>
                    </td>
                    <td>
                        <div class="bv-actions">
                            <a href="{{ route('admin.categories.edit', $categories->id) }}" class="bv-btn-edit">
                                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
                                    <path d="M11.5 2.5l2 2L5 13H3v-2L11.5 2.5z"/>
                                </svg>
                                Edit
                            </a>
                            <form method="POST" action="{{ route('admin.categories.destroy', $categories->id) }}" style="display:inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bv-btn-delete"
                                    onclick="return confirm('Delete {{ addslashes($categories->name) }}?')">
                                    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
        ...
        <div class="bv-empty">
            ...
            <div class="bv-empty-title">No Event yet</div>
            <p class="bv-empty-sub">Click <strong>Add Even</strong> to create your first event .</p>
        </div>
        @endif
    </div>
...
<div id="categoryModal" class="bv-modal-backdrop">
    <div class="bv-modal">
        <div class="bv-modal-header">
            <span class="bv-modal-title">Add <em>Category</em></span>
            ...
            <div class="bv-modal-body">
                ...
                <div class="bv-field">
                    ...
                    <input id="eventName" name="name" type="text"
                           class="bv-input" placeholder="e.g. Photography, Catering, Vediography…"
                           required>
                </div>
                ...
                <div class="bv-field" style="margin-bottom:0">
                    ...
                    <textarea id="eventDesc" name="description"
                              class="bv-textarea"
                              placeholder="Briefly describe this supplier type…"></textarea>
                </div>
