<x-app-layout>
    ...
    <div class="page-content">
        ...
        <div class="bv-page-header">
            <div>
                <h1 class="bv-page-title">Event <em>Locations</em></h1>
                <p class="bv-page-sub">Manage Event location</p>
            </div>
        ...
        <div class="bv-card">
            ...
            <table class="bv-table">
                ...
                <tbody>
                    @foreach($locations as $i => $locations)
                    <tr>
                        <td><span class="bv-row-num">{{ $i + 1 }}</span></td>
                        <td>
                            <div class="bv-locations-name">
                                <span class="bv-locations-dot"></span>
                                {{ $locations->name }}
                            </div>
                        </td>
                        <td>
                            <span class="bv-locations-desc" title="{{ $locations->description }}">
                                {{ $locations->description ?: '—' }}
                            </span>
                        </td>
                        <td>
                            <div class="bv-actions">
                                <a href="{{ route('admin.location.edit', $locations->id) }}" class="bv-btn-edit">
                                    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
                                        <path d="M11.5 2.5l2 2L5 13H3v-2L11.5 2.5z"/>
                                    </svg>
                                    Edit
                                </a>
                                <form method="POST" action="{{ route('admin.location.destroy', $locations->id) }}" style="display:inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bv-btn-delete"
                                        onclick="return confirm('Delete {{ addslashes($locations->name) }}?')">
                                        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
            ...
            <div class="bv-empty">
                ...
                <div class="bv-empty-title">No locations yet</div>
                <p class="bv-empty-sub">Click <strong>Add location</strong> to create your first location.</p>
            </div>
            @endif
        </div>
    ...
    {{-- Add location Modal --}}
    <div id="locationModal" class="bv-modal-backdrop">
        <div class="bv-modal">
            <div class="bv-modal-header">
                <span class="bv-modal-title">Add <em>location</em></span>
            ...
            <form method="POST" action="{{ route('admin.location.store') }}">
                ...
                <div class="bv-modal-body">
                    ...
                    <div class="bv-field">
                        <label class="bv-label" for="locationName">Location Name</label>
                        <input id="locationName" name="name" type="text"
                            class="bv-input" placeholder="e.g. Hinoba-an,Sipalay…"
                            required>
                    </div>

                    <div class="bv-field" style="margin-bottom:0">
                        <label class="bv-label" for="locationDesc">Description</label>
                        <textarea id="locationDesc" name="description"
                                class="bv-textarea"
                                placeholder="Briefly describe this locations"></textarea>
                    </div>
