...
<div class="page-content">
    ...
    <div class="bv-page-header">
        <a href="{{ route('admin.event.list') }}" class="bv-back-btn">
        ...
        <div>
            <h1 class="bv-page-title">Edit <em>Event Categories</em></h1>
            <p class="bv-page-sub">Update Event Category details for {{ $eventcategory->name }}</p>
        </div>
    </div>
    ...
    <div class="bv-form-layout">
        ...
        {{-- ── LEFT: Main form ── --}}
        <div>
            <form method="POST" action="{{ route('admin.event.update', $eventcategory->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="bv-card">
                    <div class="bv-card-header">
                        ...
                        <div>
                            <div class="bv-card-header-title">Categories Details</div>
                            <div class="bv-card-header-sub">Basic information about this event eventcategory</div>
                        </div>
                    </div>
                    ...
                    <div class="bv-card-body">
                        ...
                        {{-- Name --}}
                        <div class="bv-field">
                            <label class="bv-label" for="categoryName">
                                Categories Name
                        ...
                        <div class="bv-field">
                            ...
                            <textarea
                                ...
                                placeholder="Briefly describe this eventcategory — mood, style, typical décor…"
                            >{{ old('description', $eventcategory->description) }}</textarea>
                            @error('description')
                                <div class="bv-error-msg">
                                    <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2">
                                        <circle cx="6" cy="6" r="5"/>
                    ...
                    <div class="bv-form-actions">
                        <span style="font-size:0.72rem;color:#C0B8B0;">
                            Last updated: {{ $eventcategory->updated_at->diffForHumans() }}
                        </span>
                        <div class="bv-form-actions-right">
                            <a href="{{ route('admin.event.list') }}" class="bv-btn-cancel">
                                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div>
            ...
            <div class="bv-meta-card">
                ...
                <div class="bv-meta-body">
                    ...
                    <div class="bv-meta-row">
                        <span class="bv-meta-key">Categories ID</span>
                        <span class="bv-meta-val">#{{ $eventcategory->id }}</span>
                    </div>
                    <div class="bv-meta-row">
                        <span class="bv-meta-key">Created</span>
                        <span class="bv-meta-val">{{ $eventcategory->created_at->format('M d, Y') }}</span>
                    </div>
                    <div class="bv-meta-row">
                        <span class="bv-meta-key">Last Modified</span>
                        <span class="bv-meta-val">{{ $eventcategory->updated_at->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bv-danger-card">
                ...
                <div class="bv-danger-body">
                    <p class="bv-danger-desc">
                        Permanently deletes <strong>{{ $eventcategory->name }}</strong> and removes it from all associated events. This action cannot be undone.
                    </p>
                    <form method="POST" action="{{ route('admin.event.destroy', $eventcategory->id) }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bv-btn-delete"
                            onclick="return confirm('Delete {{ addslashes($eventcategory->name) }}? This cannot be undone.')">
