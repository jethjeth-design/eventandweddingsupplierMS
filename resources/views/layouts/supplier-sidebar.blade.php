@php
    ...
    $bookingCount = \App\Models\Booking::where('supplier_id', auth()->user()->supplier->id)
        ->where('status', 'pending')
        ->count();

    $pendingBidCount = \App\Models\Bid::where('supplier_id', auth()->id())
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('supplier.dashboard') }}" class="nav-item {{ $navActive['dashboard'] ? 'active' : '' }}">
        ...
        <a href="{{ route('supplier.bookings') }}" class="nav-item {{ $navActive['bookings'] ? 'active' : '' }}">
        ...
        <a href="{{ route('supplier.bids.index') }}" class="nav-item {{ $navActive['inquiries'] ? 'active' : '' }}">
            ...
            <span class="nav-tooltip">Bids</span>
        </a>

        <div class="sidebar-divider"></div>
        <div class="nav-group-label">Business</div>

        <a href="{{ route('supplier.package.index') }}" class="nav-item {{ $navActive['packages'] ? 'active' : '' }}">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7">
        ...
        <a href="#" class="nav-item {{ $navActive['earnings'] ? 'active' : '' }}">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7">
        ...
        <a href="#" class="nav-item {{ $navActive['payouts'] ? 'active' : '' }}">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7">
        ...
        <div class="nav-item nav-item--has-dropdown {{ $settingsParentActive ? 'active' : '' }} {{ $settingsDrawerOpen ? 'drawer-open' : '' }}"
            id="settingsToggle" onclick="toggleSettings()">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7">
                <circle cx="10" cy="10" r="3" />
                <path
                    d="M10 1v2M10 17v2M1 10h2M17 10h2M3.2 3.2l1.4 1.4M15.4 15.4l1.4 1.4M3.2 16.8l1.4-1.4M15.4 4.6l1.4-1.4" />
            </svg>
            <span class="nav-label">Settings</span>
            <span class="nav-tooltip">Settings</span>
            <svg class="settings-chevron" viewBox="0 0 16 16" fill="none" stroke="currentColor"
                stroke-width="2.2">
                <path d="M4 6l4 4 4-4" />
            </svg>
        </div>
            ...
            <a href="{{ route('supplier.supplierprofile') ?? '#' }}" class="sidebar-user">
                <div class="sidebar-user-avatar">
                    @if ($supplier && $supplier->photo)
                        <img src="{{ asset('storage/' . $supplier->photo) }}" alt="{{ $displayName }}">
                    @else
                        {{ $initials ?: 'S' }}
                    @endif
                </div>
