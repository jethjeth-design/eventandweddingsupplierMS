...
<head>
    ...
    <title>Suppliers — WES TEAM</title>
    ...
    <style>
        ...
        *::after {
        ...
        .sp-check-item input:checked+.sp-check-box {
            background: var(--gold);
            border-color: var(--gold);
        }

        .sp-check-item input:checked+.sp-check-box svg {
            display: block;
        }
        ...
        .sp-rating-item:hover {
            background: rgba(201, 168, 76, 0.05);
        }

        .sp-rating-item input {
            display: none;
        }
        ...
        .sp-rating-item input:checked~.sp-radio-dot {
            border-color: var(--gold);
        }

        .sp-rating-item input:checked~.sp-radio-dot::after {
            content: '';
            width: 6px;
            height: 6px;
            border-radius: 50%;
        ...
        .sp-filter-btn-mobile svg {
            width: 13px;
            height: 13px;
        }

        .sp-filter-btn-mobile:hover {
        ...
        .sp-mobile-sidebar-close:hover {
            background: var(--border-md);
        }

        .sp-mobile-sidebar-close svg {
            width: 13px;
            height: 13px;
        }
...
<body>
    ...
    <div class="mobile-menu" id="mobileMenu">
        ...
        <a href="{{ route('welcomepage.event') }}" onclick="closeMenu()">Events</a>
        <a href="{{ route('welcomepage.package') }}"
    ...
    <div class="page-hero">
        <div class="hero-inner">
            ...
            <div class="hero-search-bar">
                ...
                <input type="text" id="spSearch" class="hero-search-input"
                    placeholder="Search by supplier name, city, category…" autocomplete="off">
    ...
    @php
        ...
        $cities = $suppliers
            ->map(fn($s) => $s->supplierProfile->city ?? ($s->city ?? null))
            ->filter()
            ->unique()
            ->sort()
            ->values();
    ...
    <div class="popular-cats-bar">
        ...
        <div class="popular-cats-row" id="popCatsRow">
            @foreach ($allCatsList as $cat)
                ...
                <button class="pop-cat-chip" data-cat="{{ strtolower($cat->name) }}" onclick="togglePopCat(this)">
                    {{ $cat->name }}
...
═══════════════════════════════════ --}}
                        ...
                        <div class="sp-check-list" id="mobCatList">
                            @foreach ($allCatsList as $cat)
                                ...
                                <label class="sp-check-item">
                                    <input type="checkbox" class="sp-cat-check-mob sp-cat-check"
                                        value="{{ strtolower($cat->name) }}" onchange="applyFilters()">
                ...
                <div class="sp-filter-group">
                    <div class="sp-rating-list">
                        <label class="sp-rating-item">
                            <input type="radio" name="spRatingMob" value="" checked
                                onchange="applyFilters()">
                            <span class="sp-radio-dot"></span>
                            <span class="sp-rating-label" style="font-size:0.78rem;color:var(--warm-grey);">Any
                                rating</span>
                        </label>
                        @foreach ([4, 3, 2, 1] as $minR)
                            <label class="sp-rating-item">
                                <input type="radio" name="spRatingMob" value="{{ $minR }}"
                                    onchange="applyFilters()">
                                <span class="sp-radio-dot"></span>
                                <div class="sp-stars">
                                    @for ($s = 1; $s <= 5; $s++)
            ...
            <div class="sp-sidebar-drawer">
                ...
                <div class="sp-sidebar-card">
                        ...
                        <div class="sp-filter-group">
                            ...
                            <div class="sp-check-list">
                                @foreach ($allCatsList as $cat)
                                    @php $catCount = $suppliers->filter(fn($s) => ($s->categories ?? collect())->pluck('id')->contains($cat->id))->count(); @endphp
                                    <label class="sp-check-item">
                                        <input type="checkbox" class="sp-cat-check-desk sp-cat-check"
                                            value="{{ strtolower($cat->name) }}" onchange="applyFilters()">
                                        <span class="sp-check-box">
                                            <svg viewBox="0 0 12 10">
                                                <polyline points="1,5 4,8 11,1" />
                    ...
                    <div class="sp-filter-group">
                        ...
                        <div class="sp-rating-list">
                            <label class="sp-rating-item">
                                <input type="radio" name="spRating" value="" checked
                                    onchange="applyFilters()">
                                <span class="sp-radio-dot"></span>
                                <span class="sp-rating-label" style="font-size:0.78rem;color:var(--warm-grey);">Any
                                    rating</span>
                            ...
                            @foreach ([4, 3, 2, 1] as $minR)
                                <label class="sp-rating-item">
                                    <input type="radio" name="spRating" value="{{ $minR }}"
                                        onchange="applyFilters()">
                                    <span class="sp-radio-dot"></span>
                                    <div class="sp-stars">
                                        @for ($s = 1; $s <= 5; $s++)
                ...
                <div class="sp-topbar-left">
                    ...
                    <button class="sp-filter-btn-mobile" onclick="openMobileSidebar()">
                        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8">
                            <path d="M1 3.5h14M3.5 8h9M6 12.5h4" />
                        </svg>
                        Filters
                    </button>
    ...
    <footer>
        ...
        <div class="footer-links">
            <a href="#">Privacy</a><a href="#">Terms</a><a href="#">Support</a><a
                href="#">Blog</a>
        </div>
        <div class="footer-copy">&copy; {{ date('Y') }} WES TEAM. All rights reserved.</div>
    </footer>
    ...
    <script>
        ...
        function applyFilters() {
            ...
            const city = (spFilterCity ? spFilterCity.value || '' : (spFilterCityMob ? spFilterCityMob.value || '' : ''))
                .toLowerCase();
            ...
            cards.forEach(card => {
                const nameMatch = !q || (card.dataset.name || '').includes(q) || (card.dataset.city || '').includes(
                    q) || (card.dataset.cat || '').includes(q);
            ...
            visibleCards.sort((a, b) => {
                ...
                switch (sort) {
                    case 'popular':
                        return (rB * Math.log(vB + 1)) - (rA * Math.log(vA + 1));
                    case 'rating_high':
                        return rB - rA;
                    case 'rating_low':
                        return rA - rB;
        ...
        function updateActiveTags(q, city, cats, minRat) {
            [spActiveTags, spActiveTagsMob].forEach(container => {
                if (!container) return;
                container.innerHTML = '';
                if (q) addTag(container, 'Search: ' + q, () => {
                ...
                cats.forEach(cat => addTag(container, 'Cat: ' + cat, () => {
                    document.querySelectorAll(`.sp-cat-check[value="${cat}"]`).forEach(cb => cb
                        .checked = false);
                    applyFilters();
                }));
                if (minRat) addTag(container, minRat + '★ & up', () => {
                    document.querySelectorAll(
                            'input[name="spRating"][value=""], input[name="spRatingMob"][value=""]')
        ...
        function clearAllFilters() {
            ...
            document.querySelectorAll('input[name="spRating"][value=""], input[name="spRatingMob"][value=""]').forEach(r =>
                r.checked = true);
