...
@media(max-width:768px){
    .hamburger{display:flex;}.mobile-menu{display:flex;}
    .nav-links{display:none;}nav.main-nav{padding:1rem 1.5rem;}
}
...
@media(max-width:500px){
    .fab-logo-pill{padding:0 0.7rem;gap:0;}.fab-logo-text{display:none;}
    .fab-cta-text{display:none;}.fab-cta-pill{padding:0 0.85rem;}
    .fab-inq{bottom:1.25rem;right:1.25rem;}
}

/* ══════════════════════════════════════
   SCROLLABLE MODAL — KEY CHANGES
   ─────────────────────────────────────
   1. .modal-backdrop  → display:flex, overflow hidden on page
   2. .modal-box       → flex column, max-height 92dvh, NO overflow:hidden
   3. .modal-hd        → flex-shrink:0 (never shrinks away)
   4. .modal-supplier-strip → flex-shrink:0
   5. .modal-note      → flex-shrink:0
   6. .modal-body      → flex:1, overflow-y:auto (THE scroll container)
   7. .modal-footer    → flex-shrink:0, sticky bottom
══════════════════════════════════════ */
.modal-backdrop{
...
.modal-box{
    ...
    /* Use dvh so it works on mobile browsers with address bar */
    max-height:92dvh;
    max-height:92vh; /* fallback */
    background:var(--white);
    border-radius:8px;
    box-shadow:0 24px 64px rgba(18,15,12,0.32),0 0 0 1px rgba(201,168,76,0.15);
    /* CRITICAL: flex column so children can control their own scroll */
...
/* Header — never shrinks, always visible at top */
.modal-hd{
...
/* Supplier strip — fixed below header */
.modal-supplier-strip{
...
.modal-body{
    ...
    /* CRITICAL: this is the only part that scrolls */
    overflow-y:auto;
    ...
    <div class="pf-about-card">
        ...
        <div class="pf-about-body">
            ...
            <div class="pf-about-row">
                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M8 1C5.8 1 4 2.8 4 5c0 3.5 4 9 4 9s4-5.5 4-9c0-2.2-1.8-4-4-4z"/><circle cx="8" cy="5" r="1.5"/></svg>
                <div class="pf-about-row-text">{{ implode(', ', array_filter([$supplier->city, $supplier->province])) }}<small>Location</small></div>
            </div>
            @endif
            @if($supplier->address)
            <div class="pf-about-row">
                ...
                <div class="pf-video-grid">
                    ...
                    <div class="pf-video-card">
                        ...
                        <div class="pf-video-info">
                            ...
                            @if($pf->description)<div class="pf-post-desc" style="font-size:.76rem;color:var(--warm-grey);margin-top:2px;">{{ $pf->description }}</div>@endif
            ...
            <div class="pf-about-full">
                ...
                <div class="pf-info-card">
                    ...
                    <div style="padding:1rem 1.1rem;display:flex;flex-direction:column;gap:.75rem;">
                        @if($supplier->bio)
                        <div><div class="pf-info-k">Bio</div><div class="pf-info-v" style="font-style:italic;color:var(--warm-grey);">{{ $supplier->bio }}</div></div>
                        @endif
                        @if($supplier->description)
                        <div><div class="pf-info-k">Service Description</div><div class="pf-info-v" style="color:var(--warm-grey);">{{ $supplier->description }}</div></div>
                        @endif
                    </div>
                </div>
                @endif
                <div class="pf-info-card">
                    ...
                    <div class="pf-info-card-body">
                        <div><div class="pf-info-k">Full Name</div><div class="pf-info-v">{{ trim(($supplier->first_name ?? '').' '.($supplier->last_name ?? '')) ?: '—' }}</div></div>
                        <div><div class="pf-info-k">Phone</div><div class="pf-info-v {{ !$supplier->phone ? 'nil' : '' }}">{{ $supplier->phone ?? '—' }}</div></div>
                        <div><div class="pf-info-k">City</div><div class="pf-info-v {{ !$supplier->city ? 'nil' : '' }}">{{ $supplier->city ?? '—' }}</div></div>
                        <div><div class="pf-info-k">Province</div><div class="pf-info-v {{ !$supplier->province ? 'nil' : '' }}">{{ $supplier->province ?? '—' }}</div></div>
                        <div class="pf-full-row"><div class="pf-info-k">Address</div><div class="pf-info-v {{ !$supplier->address ? 'nil' : '' }}">{{ $supplier->address ?? '—' }}</div></div>
                    </div>
                </div>
            </div>
        </div>
...
{{-- ══ FLOATING INQUIRY BUTTON ══ --}}
<button class="fab-inq" onclick="openModal()" type="button" aria-label="Send inquiry">
    <div class="fab-logo-pill">
        <svg width="18" height="18" viewBox="0 0 20 20" fill="none">
            <path d="M3 14h14l-2-7-4 4-3-6-3 6-4-4 2 7z" fill="#C9A84C" stroke="#C9A84C" stroke-width="0.5" stroke-linejoin="round"/>
            <path d="M3 16h14" stroke="#C9A84C" stroke-width="1.8" stroke-linecap="round"/>
        </svg>
        <span class="fab-logo-text">Bikol's<span>Craft</span></span>
    </div>
    <div class="fab-cta-pill">
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M14 2H2v9h5l3 3v-3h4V2z"/></svg>
        <span class="fab-cta-text">Send Inquiry</span>
    </div>
</button>
...
<div id="inqModal" class="modal-backdrop" onclick="if(event.target===this)closeModal()">
    <div class="modal-box">
            ...
            <div class="modal-hd-brand">
                <div class="modal-hd-logo">
                    <svg viewBox="0 0 20 20" fill="none">
                        <path d="M3 14h14l-2-7-4 4-3-6-3 6-4-4 2 7z" fill="#E8C97A" stroke="#E8C97A" stroke-width="0.5" stroke-linejoin="round"/>
                        <path d="M3 16h14" stroke="#E8C97A" stroke-width="1.8" stroke-linecap="round"/>
                    </svg>
                </div>
                <div class="modal-hd-info">
                    <div class="modal-hd-title">Send an Inquiry</div>
                    <div class="modal-hd-sub">Bikol's Craft · Verified Supplier</div>
                </div>
            </div>
            <button class="modal-x" onclick="closeModal()">&#215;</button>
        </div>

        {{-- FIXED: Supplier strip --}}
        <div class="modal-supplier-strip">
            <div class="modal-sup-avatar">
                @if($supplier->photo)
            ...
            <div class="modal-sup-info">
                <div class="modal-sup-name">{{ $supplier->business_name ?? trim(($supplier->first_name ?? '').' '.($supplier->last_name ?? '')) }}</div>
                <div class="modal-sup-cat">
                    @if($supplier->city) {{ $supplier->city }}{{ $cats->count() ? ' · ' : '' }}@endif
                    {{ $cats->pluck('name')->implode(', ') }}
                </div>
            </div>
        </div>
        ...
        <div class="modal-note">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="8" cy="8" r="6"/><path d="M8 7v4M8 5.5v.5"/></svg>
            <span>Your message goes directly to the supplier. <strong>Expect a reply within 24–48 hours.</strong></span>
        </div>

        {{-- SCROLLABLE: Form body --}}
        <form action="{{ route('inquiry.store') }}" method="POST" id="inqForm" onsubmit="handleInquiry(event)" style="display:contents;">
            @csrf
            ...
            <div class="modal-body">
                <div class="fm-grid">
                    <div class="fm-field">
                        <label class="fm-lbl">First name <span style="color:#E07B5A;">*</span></label>
                        <input class="fm-in" type="text" name="first_name" placeholder="Maria" required>
                    </div>
                    <div class="fm-field">
                        <label class="fm-lbl">Last name <span style="color:#E07B5A;">*</span></label>
                        <input class="fm-in" type="text" name="last_name" placeholder="Santos" required>
                    </div>
                </div>
                <div class="fm-grid">
                    <div class="fm-field">
                        <label class="fm-lbl">Email <span style="color:#E07B5A;">*</span></label>
                        <input class="fm-in" type="email" name="email" placeholder="you@email.com" required>
                    </div>
                    <div class="fm-field">
                        <label class="fm-lbl">Phone <span style="font-weight:300;color:#C0B8B0;">(optional)</span></label>
                        <input class="fm-in" type="tel" name="phone" placeholder="+63 912 345 6789">
                    </div>
                </div>
                <div class="fm-field">
                    <label class="fm-lbl">Subject <span style="color:#E07B5A;">*</span></label>
                    <input class="fm-in" type="text" name="subject" placeholder="What do you need help with?" required>
                </div>
                <div class="fm-field">
                    <label class="fm-lbl">Message <span style="color:#E07B5A;">*</span></label>
                    <textarea class="fm-in" name="message" placeholder="Tell us about your event, date, location, guest count, and any special requirements…" required></textarea>
                </div>
                {{-- Extra padding so last field isn't flush against footer --}}
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-send" id="btnSend">
                    ...
                    <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M2 7h10M8 3l4 4-4 4"/></svg>
                </button>
            </div>
        </form>
...
<footer>
    ...
    <script>
        ...
        /* ── MODAL ── */
        function openModal() {
            document.getElementById('inqModal').classList.add('open');
            document.body.style.overflow = 'hidden';
        }
        function closeModal() {
            document.getElementById('inqModal').classList.remove('open');
            document.body.style.overflow = '';
        }
        document.addEventListener('keydown', e => {
            if (e.key === 'Escape') { closeModal(); lbClose(); }
        });
        ...
        function handleInquiry(e) {
            ...
            btn.disabled = true;
            btn.innerHTML = '<span>Sending…</span><svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 2v10M2 7h10"/></svg>';
            ...
            .then(() => {
                closeModal();
                e.target.reset();
                showToast('success', 'Message sent! The supplier will reply within 24–48 hours.');
            })
        ...
        function lbNav(dir) { const n = lbIdx+dir; if(n>=0 && n<lbUrls.length) lbGo(n); }
        function lbClose() {
            document.getElementById('pfLb').classList.remove('open');
            if (!document.getElementById('inqModal').classList.contains('open'))
                document.body.style.overflow = '';
        }
