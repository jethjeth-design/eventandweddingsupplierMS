...
<head>
    ...
    <title>{{ $popular->name }} — WES TEAM</title>
    ...
    <style>
        ...
        *::after {
        ...
        .match-card:hover {
            box-shadow: 0 6px 28px rgba(30, 27, 24, 0.11);
            transform: translateY(-2px);
            border-color: rgba(201, 168, 76, 0.4);
        }

        .match-card.mc-hidden {
            display: none;
        }
        ...
        .mc-incl-item.matched svg {
            color: var(--green);
        }

        .mc-incl-item.unmatched svg {
            color: var(--border-md);
        }
...
<body>
    ...
    <nav class="main-nav">
        ...
        <div class="nav-links">
            ...
            <a href="{{route('welcomepage.event')}}" class="{{ request()->routeIs('events*') ? 'nav-active' : '' }}">Events</a>
    ...
    <div class="mobile-menu" id="mobileMenu">
        ...
        <a href="{{ route('welcomepage.package') }}"
            class="{{ request()->routeIs('welcomepage.package') ? 'nav-active' : '' }}"
    ...
    <div class="page-hero">
        <div class="hero-inner">
            ...
            <a href="{{ route('welcomepage.package') }}" class="hero-back">
            ...
            <div class="match-grid" id="matchGrid">
                ...
                @forelse($matchedPackages as $idx => $package)
                    @php
                        ...
                        $matchCount = $package->inclusions
                            ->filter(fn($inc) => in_array(strtolower(trim($inc->type ?? '')), $targetTypes))
                            ->count();
                        $totalInc = $package->inclusions->count();
                        $matchPct =
                            $totalInc > 0 ? round(($matchCount / max($totalInc, count($targetTypes))) * 100) : 0;
                        ...
                        $cardTypes = $package->inclusions
                            ...
                            ->implode('|');
                    @endphp
                    <div class="match-card" data-types="{{ $cardTypes }}"
                                ...
                                <div class="mc-incl-list">
                                    @foreach ($package->inclusions as $inc)
                                        ...
                                        <div class="mc-incl-item {{ $isMatch ? 'matched' : 'unmatched' }}">
                                            ...
                                            @if ($inc->type)
                                                <span
                                                    style="font-size:0.6rem;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;padding:1px 5px;border-radius:3px;background:{{ $isMatch ? 'var(--green-light)' : 'var(--ivory)' }};color:{{ $isMatch ? 'var(--green)' : 'var(--warm-grey)' }};border:1px solid {{ $isMatch ? 'var(--green-border)' : 'var(--border-md)' }};flex-shrink:0;margin-left:auto;white-space:nowrap;">
                                                    {{ ucfirst($inc->type) }}
                                                </span>
                                            @endif
                                        </div>
                        ...
                        <div class="mc-foot">
                            ...
                            <a href="#" class="pp-view-btn"
                                style="display:inline-flex;align-items:center;gap:0.3rem;padding:0.38rem 0.85rem;border-radius:6px;border:1.5px solid var(--border-md);background:var(--white);font-family:var(--font-body);font-size:0.72rem;font-weight:500;color:var(--charcoal);text-decoration:none;transition:border-color 0.2s,background 0.2s,color 0.2s;white-space:nowrap;">
                                View
                ...
                @empty
                    <div class="empty-state">
                        ...
                        <p>No supplier packages match the inclusion types of this popular package yet.<br>Check back
                            soon as suppliers add more offerings.</p>
                    </div>
                @endforelse
