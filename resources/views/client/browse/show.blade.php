<x-client-layout>
    ...
    ══════════════════════════════ */
        ...
        .sp-pkg-list-item:last-child {
            border-bottom: none;
        }

        .sp-pkg-list-item:hover {
            background: var(--ivory);
        }

        .sp-pkg-list-item.active {
        ...
        .sp-pkg-list-item.active .sp-pkg-list-dot {
            background: var(--gold);
        }
    ...
    ══════════════════════════════ */
        ...
        .sp-rev-card:hover {
            box-shadow: 0 4px 20px rgba(30, 27, 24, 0.08);
        }
        ...
        .sp-rev-card:nth-child(1) {
            animation-delay: 0s;
        }

        .sp-rev-card:nth-child(2) {
            animation-delay: .05s;
        }

        .sp-rev-card:nth-child(3) {
            animation-delay: .10s;
        }

        .sp-rev-card:nth-child(4) {
            animation-delay: .15s;
        }
    ...
    ══════════════════════════════ */
        ...
        .sp-fab-btn:hover {
            transform: scale(1.1);
            box-shadow: 0 6px 24px rgba(30, 27, 24, 0.28);
        }
...
══════════════════════════════ */
        ...
        .btn-ev-resend:disabled {
            opacity: 0.5;
            cursor: not-allowed;
            transform: none !important;
        }

        .btn-ev-resend:not(:disabled):hover {
            background: var(--gold-light);
            transform: translateY(-1px);
        }
            ...
            <div class="sp-pkg-layout">
                ...
                <div class="sp-pkg-detail">
                    @foreach ($packages as $index => $package)
                        ...
                        <div class="sp-pkg-panel {{ $index === 0 ? 'active' : '' }}"
                            id="pkg-panel-{{ $package->id }}">
            ...
            @if ($ratingCount)
                <div class="sp-reviews-summary">
                    ...
                    <div class="sp-rev-avg-block">
                        ...
                        <div class="sp-rev-avg-label">{{ $ratingCount }} review{{ $ratingCount !== 1 ? 's' : '' }}
                        </div>
                    </div>
                            ...
                            <div class="sp-rev-bar-row">
                                ...
                                <span class="sp-rev-bar-star">
                                    <svg viewBox="0 0 24 24"
                                        style="fill:#C9A84C;stroke:#C9A84C;stroke-width:1.4;stroke-linejoin:round;">
                                        <polygon
                                            points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
                                    </svg>
                                </span>
                        ...
                        <div class="sp-rev-card rating-{{ $rVal }}">
                            ...
                            <div class="sp-rev-card-top">
                                ...
                                <div class="sp-rev-user">
                                    ...
                                    @if ($rate->created_at)
                                        <div class="sp-rev-date">
                                            {{ \Carbon\Carbon::parse($rate->created_at)->format('M d, Y') }}</div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div
                                style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:0.4rem;">
...
══════════════════════════════ --}}
    <div class="sp-fab-group">
        <div class="sp-fab-item" style="position:relative;">
            ...
            <a href="{{ route('client.supplier.calendar', $supplier->id) }}" class="sp-fab-btn fab-cal"
                ...
                <div class="bv-modal-body">
                    ...
                    @if ($events->isEmpty())
                        <div class="bv-alert-warn">
                            ...
                            <span>You need to create an event first before you can make a booking. Go to your dashboard
                                and create an event.</span>
                        </div>
                    @endif
    ...
    <div id="emailVerificationModal" class="ev-modal-backdrop">
        <div class="ev-modal">
            ...
            <div class="ev-modal-footer">
                ...
                <button type="button" class="btn-ev-close-modal"
                    onclick="closeEmailVerificationModal()">Close</button>

                <form method="POST" action="{{ route('verification.send') }}" id="ev-resend-form"
                    style="margin:0;">
                    @csrf
                    <button type="submit" class="btn-ev-resend" id="ev-resend-btn" onclick="handleEvResend(event)">
                        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2">
